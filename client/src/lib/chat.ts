...
export function SetSSEListener(es: EventSource, messagesDispatch: React.Dispatch<React.SetStateAction<any[]>>) {
    es.addEventListener("message", (event) => {
        try {
            ...
            messagesDispatch(prevMessages => [...prevMessages, {
                author: data.sender,
                timestamp: data.time,
                message: data.content,
                variant: data.sender === username.getState().name ? "send" : "receive",
...
export function sendMessage(message: string) {
    ...
    if (typeof username.getState().name === "undefined" || username.getState().name === "") {
